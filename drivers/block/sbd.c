/* NOTE: obtained from http://blog.superpat.com/2010/05/04/a-simple-block-driver-for-linux-kernel-2-6-31/ */
...
//	struct crypto_cipher *sbdCipher;	
	//sbdCipher = crypto_alloc_cipher("sha1", 0, 0);
} Device;
...
static void sbd_transfer(struct sbd_device *dev, sector_t sector,
	...
	unsigned int cipherBlockSize = crypto_cipher_blocksize(n_crypt);
	...
	u8 *datagroup2;
	int i;	
	crypto_cipher_setkey(n_crypt, key, 11);
	...
	if (write) {
		...
		for (i = 0; i < nbytes; i+=cipherBlockSize) 
			crypto_cipher_encrypt_one(n_crypt, datagroup1 + i, datagroup2 + i);
...
/* changed from:sbd_transfer(&Device, blk_rq_pos(req), blk_rq_cur_sectors(req),
*	req->buffer, rq_data_dir(req)); to:
* 	(as per comment on http://blog.superpat.com/2010/05/04/a-simple-block-driver-for-linux-kernel-2-6-31/)
*/
...
static int __init sbd_init(void) {
	/*
  	...
  	*/
  	n_crypt = crypto_alloc_cipher("sha1", 0, 0);
